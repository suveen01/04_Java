...
public class StudentService {
	 ...
	 * (students에 저장된 주소가 복사되서 반환됨 == 얕은 복사 개념)
	 * 
	 * @return students
	 */
	...
	public boolean addStudent(StudentDTO student) {
		...
		// students 객체 배열 요소 중 비어있는(null) 인덱스 찾기
		for (int i = 0; i < students.length; i++) {

			if (students[i] == null) {
				// i번째 인덱스 요소가 참조하는게 없을 경우 전달 받은 StudentDTO 객체 추가 후 true 반환
				students[i] = student;
				return true;
			}
		}
	...
	public StudentDTO selectName(String targetName) {
		...
		for(StudentDTO std : students) {
			...
			// students 순차 접근 중 null을 만났을 때 == 학생이 없을 경우 뒤에도 학생이 없을 것이다
			if(std == null) {
				return null;
			 }
			// 학생이 있을 경우 -> 이름 비교
