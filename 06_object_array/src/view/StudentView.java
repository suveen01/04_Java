...
public class StudentView {
	...
	private void allStudent() {
		...
		for(StudentDTO std : students) {
			...
			/* 	참조 변수를 문자열 만들 때, 또는 print() 관련 메서드에 사용될 때 참조 변수에 저장된 주소가 출력되면 이상하니까 컴파일러가 자동으로 참조변수 .toString을 호출하도록 변경
			 */
//			System.out.println(std); -> 가능
	...
	/** 학생 추가 view 메서드
	 * <pre>
	 * 학번, 이름, 성별을 입력받아 StudentDTO 객체로 만들어서 StudentService.addStudent() 메서드에 전달
	...
	private void selectScore() {
		...
		// 정상일 경우
		// 해당 인덱스 번째 학생(StudentDTO)을 반환받기
		
		StudentDTO std = service.selectIndex(index);
		// -> 무조건 참조하는 학생 객체가 존재할 수 밖에 없음
	...
	/** 평균 최고점, 최저점 학생
	 * <pre>
	 * 	String StudentService.selectMaxMin() 메서드를 호출하여 아래와 같은 문자열을 반환 받아 출력하기
	 * 
	 * -------------------
	 * 최고점 : 유리(85.4)
	 * 최저점 : 짱구(31.6)
...
//		if(confirm == 'N') {} // char 로 했을 때 대략적 형태
