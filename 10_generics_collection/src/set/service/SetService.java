...
//		인터페이스 					구현제
	...
	/**
	 * Hash 단어가 포함된 컬렉션은 무조건 equals(), hashCode()가 오버라이딩 되어있어야함
	 * 
	...
	public void test2() {
		...
		// 1. equals() 오버라이딩 x, hashCode() 오버라이딩 x -> 중복 제거 안됨 O
		// 2. equals() 오버라이딩 o, hashCode() 오버라이딩 x -> 중복 제거 안됨 O
		// 3. equals() 오버라이딩 x, hashCode() 오버라이딩 o -> 중복 제거 안됨 O
		// 4. equals() 오버라이딩 o, hashCode() 오버라이딩 o -> 중복 제거 성공
	}
	 ...
	 * <E>반복자가 반환하는 요소의 유형
	 * 
	 * 1) boolean hasNext() : 다음 반복할 요소가 있으면 true, 없으면 false
	 * 
	 * 2) E next() : 다음 요소를 반환
	 *  -> 무조건 hasNext() 이후에 사용 
	 * 
	...
	public void test3() {
		...
		// snacks(set)에 저장된 값을 순차접근(반복접근) 하는 객체 반홚받기
		Iterator<String> it = snacks.iterator();
		
		// 반복접근할 객체(꺼낼 객체)가 존재하면 반복
		while(it.hasNext()) {
...
//		-> snacks Set 에 저장된 데이터를 이용해 ArrayList 생성
