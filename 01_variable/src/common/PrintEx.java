...
public class PrintEx {
	public static void main(String[]args){
		 ...
		 *  	(ln == line)
		 * 
		 * 3) System.out.printf("문자열 템플릿(패턴)", 패턴에 대입될 값/변수가 여러개 작성 가능);
		 * 	- 정해진 형식(템플릿)에 맞는 문자열을 출력하는 구문
		 * 	- 문자열 템플릿 중간에 "%" 로 시작하는 패턴을 입력하여 원하는 값/변수를 대입할 수 있다.
		 * 
		 * [패턴 종류]
		 ...
		 * 		ex) %-4d, 3 대입 -> 3ㅁㅁㅁ (ㅁ은 빈칸 표시)  
		 * 
		 * 
		 * - 소수점 : 소수점 아래 몇째 자리까지 표시할지 지정
		 * 						(지정된 자리 아래에서 반올림 처리)
		 * 		ex) %.1f, 3.15 대입 -> 3.2 (3.15에서 반올림됨)
		 * 
 		 */
...
//		신짱구는 5세 남아로 키는 80.5 이며, 자바 공부 여부 : false
