...
public class PolymorphismService {
	...
	/* [매개 변수의 다형성]
	 * 
	 * - 매개 변수의 타입을 "부모 타입"으로 설정하여 모든 자식 객체 참조 주소를 전달 받을 수 있도록 함(다형성 - 업캐스팅 적용)
	 * 
	...
	public void printObject(Object obj) {
		...
		if(obj instanceof Child) {
			Child c = (Child)obj; // 다운캐스팅 진행
			
			result = "[Child Type]\n";
			result += c.getLastName() + " / " + c.getMoney() + " / " + c.getCar();
			
		}
		else if(obj instanceof Parent) { //obj가 참조하는 객체의 타입이 Parent 또는 Parent를 상속 받은 자식 타입인 경우
			Parent p = (Parent)obj; // 다운캐스팅 진행
			
			result = "[Parent Type]\n";
			result += p.getLastName() + " / " + p.getMoney();
		} 
		/* else if(obj instanceof Object) - 모든 클래스/객체는 Object의 자식이기 때문에 else에 다 걸림 */  
		else { //obj가 참조하는 객체의 타입이 Object 또는 Object를 상속 받은 자식 타입인 경우
	...
	/* [반환형의 다형성]
	 * - 메서드의 반환형을 부모 타입을 설정하여 return 되는 자식 객체의 주소를 참조할 수 있게 함
	 * - 다형성의 업캐스팅 적용
	 */
...
//		매개변수의 다형성 + 반환형의 다형성
	...
	public void test3() {
		...
		Child c1 = new Child("최", 10, "캐스터"); // 자식 = 자식
